...
def _llm_offer_response(question: str) -> dict:
    return {
        "answer": (
            ...
            "다만, 원하신다면 버니(BUNI)가 LLM을 활용하여 일반적인 관점에서\n" 
            "내용을 정리하고 답변을 제공할 수 있습니다.\n"
            "진행할까요?\n" 
            "`예` 또는 `아니오`로 답해 주세요."
        ),
        "sources": [],
...
def _llm_fallback_answer(question: str) -> str:
    prompt = f"""
너는 QA팀을 지원하는 실무형 챗봇 BUNI다.

현재 질문은 QA Notion 검색 결과에서 근거를 찾지 못했다.
사용자가 일반 답변 제공에 동의했으므로 일반 지식 기반으로만 답변한다.
...
2. 답변 원칙
- 실제 정책이나 구현이라고 단정하지 않는다.
- "일반적으로", "보통은", "확인 관점에서는" 등의 표현을 사용한다.
- QA Notion에 없는 내용을 추측하여 사실처럼 말하지 않는다.

3. 결함 또는 테스트 관련 질문인 경우
아래 형식으로 답변한다.
...
중요:
질문이 결함, 버그, 테스트, QA, 검증, 재현, 배포, 회귀, 시나리오와 관련된 경우에만 QA 관점으로 답변한다.
그 외 질문은 일반적인 답변을 제공한다.
